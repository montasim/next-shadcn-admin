 ...
 * 
 * Step 2 of password reset: Verify OTP and create AuthSession
 * 
 * Security:
 * - OTP must match email + RESET_PASSWORD intent
 * - OTP must be unused and unexpired
 * - Single-use enforcement via transaction
 * - Creates AuthSession for resumability (30 minutes)
 * - Invalidates older RESET_PASSWORD sessions
 */
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Find valid OTP for email and RESET_PASSWORD intent
        const otpRecord = await findValidOtp(email, AuthIntent.RESET_PASSWORD)
        ...
        await prisma.$transaction(async (tx) => {
            ...
            // Delete existing RESET_PASSWORD sessions for this email
