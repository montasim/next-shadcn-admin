...
export default function AdminCategoryDetailsPage() {
  ...
  return (
    <div className="bg-background h-screen overflow-y-auto no-scrollbar pb-4">
      {/* Action Buttons */}
      <div className="flex lg:justify-end justify-between gap-2 mb-4">
        <Button variant="outline" size="sm" onClick={() => router.push(`/categories/${category.id}`)}>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4 text-sm">
              <MetadataItem label="Total Spend:" value={`৳${category.analytics?.totalSpend?.toLocaleString() || 0}`} />
              <MetadataItem label="Hard Copy:" value={category.analytics?.booksByType?.HARD_COPY || 0} />
              <MetadataItem label="eBook:" value={category.analytics?.booksByType?.EBOOK || 0} />
              <MetadataItem label="Audio:" value={category.analytics?.booksByType?.AUDIO || 0} />
              <MetadataItem label="Total Books:" value={category.analytics?.totalBooks || 0} />
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 lg:gap-4">
              <AdminMetadataItem
                label="ID:"
                value={<span className="font-mono">{category.id.slice(0, 8)}...</span>}
              />
              {category.createdAt && (
                <AdminMetadataItem
                  label="Created:"
                ...
                <AdminMetadataItem label="Entered By:" value={
                  <div className="flex items-center gap-2 -mt-1">
                    <Avatar className="h-4 w-4">
                      <AvatarImage src={category.entryBy.directAvatarUrl} />
                      <AvatarFallback className="text-[10px]">
                        {category.entryBy.name?.[0] || category.entryBy.username?.[0] || 'U'}
                      </AvatarFallback>
                    </Avatar>
                    <UserLinkButton user={category.entryBy} />
                  </div>
                } />
              )}
            </div>
            ...
            <DashboardSummary
              summaries={[
                ...
                },
                {
                  ...
                  value: category.analytics?.completedReaders || 0,
                  description: 'Finished reading',
                  icon: BookOpen,
                },
                {
                  title: 'Avg Completion',
                  value: category.analytics?.totalReaders
                    ? `${Math.round((category.analytics.completedReaders / category.analytics.totalReaders) * 100)}%`
                    : 'N/A',
...
function BooksByCategoryTab({ categoryId }: { categoryId: string }) {
  const { data, isLoading } = useSWR(`/api/admin/categories/${categoryId}/books`, (url) => fetch(url).then(r => r.json()))
...
function AnalyticsTab({ categoryId }: { categoryId: string }) {
  const { data, isLoading: isLoadingChart } = useSWR(`/api/admin/categories/${categoryId}/visits?chart=true`, (url) => fetch(url).then(r => r.json()))
  const { data: visitsData, isLoading: isLoadingVisits } = useSWR(`/api/admin/categories/${categoryId}/visits?limit=10`, (url) => fetch(url).then(r => r.json()))
...
function ReadersTab({ categoryId }: { categoryId: string }) {
  const { data, isLoading } = useSWR(`/api/admin/categories/${categoryId}/readers`, (url) => fetch(url).then(r => r.json()))
