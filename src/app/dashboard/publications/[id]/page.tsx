...
export default function AdminPublicationDetailsPage() {
  ...
  return (
    <div className="bg-background h-screen overflow-y-auto no-scrollbar pb-4">
      {/* Action Buttons */}
      <div className="flex lg:justify-end justify-between gap-2 mb-4">
        <Button variant="outline" size="sm" onClick={() => router.push(`/publications/${publication.id}`)}>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4 text-sm">
              ...
              <MetadataItem label="Total Books:" value={publication.analytics?.totalBooks || 0} />
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 lg:gap-4">
              <AdminMetadataItem
                label="ID:"
                value={<span className="font-mono">{publication.id.slice(0, 8)}...</span>}
              />
              {publication.createdAt && (
                <AdminMetadataItem
                  label="Created:"
                  value={new Date(publication.createdAt).toLocaleString()}
                />
              )}
              {publication.updatedAt && (
                <AdminMetadataItem
                  label="Last Updated:"
                  value={new Date(publication.updatedAt).toLocaleString()}
                />
              )}
              {publication.entryBy && (
                <AdminMetadataItem label="Entered By:" value={
                  <div className="flex items-center gap-2 -mt-1">
                    <Avatar className="h-4 w-4">
                      <AvatarImage src={publication.entryBy.directAvatarUrl} />
                      <AvatarFallback className="text-[10px]">
                        {publication.entryBy.name?.[0] || publication.entryBy.username?.[0] || 'U'}
                      </AvatarFallback>
                    </Avatar>
                    <UserLinkButton user={publication.entryBy} />
                  </div>
                } />
              )}
            </div>
            ...
            <DashboardSummary
              summaries={[
                ...
                },
                {
                  ...
                  value: publication.analytics?.completedReaders || 0,
                  description: 'Finished reading',
                  icon: BookOpen,
                },
                {
                  title: 'Avg Completion',
                  value: publication.analytics?.totalReaders
                    ? `${Math.round((publication.analytics.completedReaders / publication.analytics.totalReaders) * 100)}%`
                    : 'N/A',
...
function BooksByPublicationTab({ publicationId }: { publicationId: string }) {
  const { data, isLoading } = useSWR(`/api/admin/publications/${publicationId}/books`, (url) => fetch(url).then(r => r.json()))
...
function AnalyticsTab({ publicationId }: { publicationId: string }) {
  const { data, isLoading: isLoadingChart } = useSWR(`/api/admin/publications/${publicationId}/visits?chart=true`, (url) => fetch(url).then(r => r.json()))
  const { data: visitsData, isLoading: isLoadingVisits } = useSWR(`/api/admin/publications/${publicationId}/visits?limit=10`, (url) => fetch(url).then(r => r.json()))
