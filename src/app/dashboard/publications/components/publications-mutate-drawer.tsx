...
import {
  checkPublicationNameAvailability,
} from '../actions'
...
type PublicationForm = z.infer<typeof formSchema>

export function PublicationsMutateDrawer({ open, onOpenChange, currentRow, onSuccess }: Props) {
  ...
  useEffect(() => {
    const checkName = async () => {
      ...
      try {
        const result = await checkPublicationNameAvailability(nameValue, currentRow?.id)
        if (result.isAvailable) {
          ...
          form.setError('name', {
            type: 'manual',
            message: result.error || 'Publication name is not available',
          })
        }
      } catch (error) {
        console.error('Publication name check failed:', error)
      } finally {
  ...
  const isFormDisabled = () => {
    ...
    // For new publications, validate all fields
    if (!isUpdate) {
  ...
  const onSubmit = async (data: PublicationForm) => {
    try {
      ...
      if (isUpdate && currentRow) {
        await updatePublication(currentRow.id, formData)
        toast({
      ...
      toast({
        title: 'Error',
        description: `Failed to ${isUpdate ? 'update' : 'create'} publication`,
        variant: 'destructive',
      })
    }
  }
    ...
    >
      <SheetContent className='flex flex-col'>
        <SheetHeader className='text-left'>
          <SheetTitle>{isUpdate ? 'Update' : 'Create'} Publication</SheetTitle>
          <SheetDescription>
            {isUpdate
              ? 'Update the publication by providing necessary info.'
              : 'Add a new publication by providing necessary info.'}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='space-y-1'>
                  <FormLabel>Publication Name <span className="text-destructive">*</span></FormLabel>
                  <FormControl>
