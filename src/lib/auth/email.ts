 ...
 */
function getEmailChangeOtpEmailTemplate(otp: string): {
  subject: string
  html: string
  text: string
} {
  return {
      ...
      <html>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #ffffff; padding: 40px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p style="font-size: 16px; color: #666;">You requested to change your email address. To complete this process, please use the verification code below:</p>

            <div style="background: #f7f7f7; border: 2px dashed #667eea; border-radius: 8px; padding: 20px; margin: 30px 0; text-align: center;">
              <p style="margin: 0 0 10px 0; color: #666; font-size: 14px; text-transform: uppercase; letter-spacing: 1px;">Email Change Code</p>
              <p style="font-size: 36px; font-weight: bold; color: #667eea; margin: 0; letter-spacing: 8px; font-family: 'Courier New', monospace;">${otp}</p>
            </div>

            <p style="font-size: 14px; color: #999; margin-top: 30px;">
              <strong>Security Note:</strong> This code expires in 10 minutes. If you didn't request an email change, please ignore this email.
            </p>
          </div>
          <div style="text-align: center; margin-top: 20px; color: #999; font-size: 12px;">
            <p>&copy; ${new Date().getFullYear()} ${APP_NAME}. All rights reserved.</p>
    ...
    `,
    text: `
${APP_NAME} - Email Change Verification
...
export async function sendEmailChangeOtp(
  ...
  const { subject, html, text } = getEmailChangeOtpEmailTemplate(otp)
